...
export const TurnoModel = {
  ...
  // 3. Obtener un turno específico por su ID
  ...
  create: async (turnoData) => {
    ...
    const query = `
      ...
      VALUES ($1, $2, $3, TO_TIMESTAMP($4, 'YYYY-MM-DD HH24:MI:SS'), 1)
      RETURNING id_turno, fecha_hora::text;
    `
  ...
  updateEstado: async (id_turno, id_estado_turno) => {
    const query = `
      ...
      WHERE id_turno = $1
      RETURNING id_turno, id_estado_turno;
    `
    const { rows } = await pool.query(query, [id_turno, id_estado_turno])
    return rows[0]
  },

  // 6. Reprogramar fecha y hora
  reprogramar: async (id_turno, nueva_fecha_hora) => {
    const query = `
      UPDATE turno
      SET fecha_hora = TO_TIMESTAMP($2, 'YYYY-MM-DD HH24:MI:SS'), id_estado_turno = 1
      WHERE id_turno = $1
      RETURNING id_turno, fecha_hora::text, id_estado_turno;
    `
    const { rows } = await pool.query(query, [id_turno, nueva_fecha_hora])
    return rows[0]
  },

  // 7. Obtener turnos por especialista
  ...
  // 8. Verificar turnos en rango
  verificarTurnosEnRango: async (id_especialista, fecha_inicio, fecha_fin) => {
    const query = `
        SELECT EXISTS (
            SELECT 1 FROM turno 
            WHERE id_especialista = $1 
            AND fecha_hora >= TO_TIMESTAMP($2, 'YYYY-MM-DD HH24:MI:SS')
            AND fecha_hora < TO_TIMESTAMP($3, 'YYYY-MM-DD HH24:MI:SS')
        ) as existe;
    `;
