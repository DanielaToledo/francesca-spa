...
import { UserPlus, Loader2, CheckCircle2, X, ArrowLeft } from 'lucide-react';
...
export default function GestionUsuarios() {
    ...
    setPassword('');
    setRol('Especialista');
    setUsuarioEditando(null); // Esto quita el modo edición
};
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-[#FDECEB] p-6 rounded-2xl border border-[#A87379]/20 h-fit">
                ...
                <div className="space-y-3">
                        ...
                        <div className="bg-white p-3 rounded-lg border border-[#A87379]/20 text-sm h-40 overflow-y-auto">
                            ...
                            {servicios.map(s => (
                                <label key={s.id_servicio} className="flex items-center gap-2 mb-1 cursor-pointer hover:bg-slate-50 p-1 rounded">
                                    <input type="checkbox" checked={serviciosSeleccionados.includes(s.id_servicio)} onChange={() => toggleServicio(s.id_servicio)} className="form-checkbox text-[#A87379] rounded focus:ring-[#A87379]" />
                                    {s.nombre_servicio}
                                </label>
                            ))}
                        </div>
                    ...
                    <div className="flex gap-3">
                        <button type="button" onClick={() => setShowConfirm(true)} className="flex-1 bg-[#A87379] text-white py-3 rounded-lg font-bold hover:bg-[#96666b] active:scale-[0.98] transition-all">
                            {usuarioEditando ? 'Guardar Cambios' : 'Dar de Alta'}
                        </button>
                        {usuarioEditando && (
                            <button type="button" onClick={resetFormulario} className="flex-1 bg-slate-100 text-slate-600 py-3 rounded-lg font-bold hover:bg-slate-200 transition-all">
                                Cancelar
                            </button>
                        )}
                    </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="text-gray-400 text-[10px] uppercase tracking-wider border-b border-gray-100">
                                <th className="pb-4 w-1/4">Nombre Completo</th>
                                <th className="pb-4 w-1/6">Rol</th>
                                <th className="pb-4 w-1/4">Servicios</th>
                                <th className="pb-4 w-1/6">Estado</th>
                                <th className="pb-4 w-1/6 text-right">Acción</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            ...
                            .map(u => (
                                <tr key={u.id_usuario} className="hover:bg-slate-50/50">
                                    <td className="py-4 font-semibold text-gray-800">{u.nombre} {u.apellido}</td>
                                    ...
                                    <td className="py-4 text-xs text-gray-500 italic max-w-[180px] truncate">
                                        {u.lista_servicios || "Sin servicios"}
                                    </td>
                                    ...
                                    <td className="py-4 text-right">
                                        <button onClick={() => iniciarEdicion(u)} className="text-slate-400 hover:text-slate-600 mr-3 text-xs font-bold">Editar</button>
                                        <button 
                                            onClick={() => u.activo ? handleBaja(u.id_usuario) : handleAlta(u.id_usuario)}
                                            className={`text-xs font-bold ${u.activo ? "text-red-500" : "text-green-600"}`}
                                        >
