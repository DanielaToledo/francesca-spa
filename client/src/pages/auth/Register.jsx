  ...
  return (
    <div className="min-h-screen bg-[#FBF9F8] flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-[#F4CFCC]/40">
        <h2 className="text-3xl font-bold text-[#A87379] text-center mb-2">Crear Cuenta ✨</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Daniela"
                className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#EAA0AB]"
              />
            </div>
            <div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#A87379] hover:bg-[#A87379]/90 text-white font-medium py-2.5 rounded-lg transition-colors disabled:bg-slate-400 mt-2"
          >
            {loading ? 'Procesando registro...' : 'Registrarme'}
          </button>
        </form>

        <div className="mt-6 pt-6 border-t border-slate-50 text-center text-sm text-slate-600">
          ¿Ya tienes una cuenta?{' '}
          <Link to="/login" className="text-[#A87379] hover:text-[#A87379]/80 font-semibold transition-colors hover:underline">
            Inicia sesión aquí
          </Link>
        </div>
      </div>
